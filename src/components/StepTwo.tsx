...
export default function StepTwo({ onGenerate, onBack, isLoading, loadingMessage = "생성 중...", error }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-display font-bold text-amber-400 mb-2">사진 업로드</h1>
      ...
      {isLoading && (
        <div className="mb-6 p-6 bg-amber-500/10 border border-amber-500/30 rounded-xl text-center">
          <div className="text-amber-400 text-sm font-medium mb-2">✨ Claude AI가 사진을 분석하고 기사를 작성 중입니다...</div>
          <div className="text-stone-500 text-xs">{loadingMessage === "사진 분석 중..." ? "📷 사진 순서 분석 중..." : "✍️ 글 생성 중... 잠시만 기다려주세요."}</div>
